...
class Member(db.Model):
  ...
  pledge = db.relationship('Pledge', backref='pledgor', foreign_keys='Pledge.member_id')
...
class Project(db.Model):
  ...
  time_create = db.Column(db.DateTime(timezone = False), nullable=False)
  pledge = db.relationship('Pledge', backref='project', foreign_keys='Pledge.project_id')

  @property
  def num_pledges(self):
    return len(self.pledge)
  ...
  def num_days_left(self):
    return (self.time_end - datetime.datetime.now()).days

  @property
  def percentage_funded(self):
      return self.total_pledges * 100 / self.goal_amount
